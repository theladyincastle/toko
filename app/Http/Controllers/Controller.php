    ...
    public function bayar($id)
    {
        ...
        $apiContext = new \PayPal\Rest\ApiContext(
            new \PayPal\Auth\OAuthTokenCredential(
                config('paypal.client_id'), // Ganti dengan Client ID PayPal Anda
                config('paypal.secret')    // Ganti dengan Secret Key PayPal Anda
            )
        );
    
        // Konfigurasi pembayar
        $payer = new Payer();
        $payer->setPaymentMethod("paypal");
    
        // Detail item
        $item = new Item();
        $item->setName('Transaksi #' . $find_data->code_transaksi)
            ->setCurrency('USD')
            ->setQuantity(1)
            ->setPrice($find_data->total_harga);
        ...
        $amount->setCurrency("USD")
            ->setTotal($find_data->total_harga);
    
        // Transaksi
        $transaction = new Transaction();
        $transaction->setAmount($amount)
            ->setItemList($itemList)
            ->setDescription("Pembayaran untuk transaksi #" . $find_data->code_transaksi);
    
        // URL pengalihan
        $redirectUrls = new RedirectUrls();
        $redirectUrls->setReturnUrl(route('paypal.success', ['id' => $id])) // URL sukses
            ->setCancelUrl(route('paypal.cancel', ['id' => $id]));          // URL batal
        ...
        try {
            ...
            // Kembali ke halaman detail transaksi dengan URL persetujuan
            return view('pelanggan.page.detailTransaksi', [
                'name' => 'Detail Transaksi',
                'title' => 'Detail Transaksi',
                'count' => $countKeranjang,
                'token' => $payment->getApprovalLink(), // Link persetujuan PayPal
                'data' => $find_data,
            ]);
        } catch (\PayPal\Exception\PayPalConnectionException $ex) {
            return back()->withErrors('Terjadi kesalahan saat memproses pembayaran: ' . $ex->getMessage());
        }
    }
...
{
    $name = 'Dashboard'; // Atau nilai sesuai kebutuhan
    return view('admin.layout.index', compact('name'));
}
